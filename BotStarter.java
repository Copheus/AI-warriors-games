			...
			if(fromRegion.ownedByPlayer(myName)) //do an attack
			{
				...
				for(int i=1;i<possibleToRegions.size();i++) {
					for(int j=0;j<possibleToRegions.size()-1;j++) {
						possibleToRegions region1=possibleToRegions.get(j);
						possibleToRegions region2=possibleToRegions.get(j+1);
						if(region1.getArmies()>region2.getArmies()) {
							possibleToRegions.set(j,region2);
							possibleToRegions.set(j+1,region1);
						} elseif(region1.getArmies()<region2.getArmies()){
				...
				{
					Region toRegion = possibleToRegions.get(1);
					
					if(!toRegion.getPlayerName().equals(myName) && fromRegion.getArmies()>toRegion.getArmies()) //do an attack
					{
						
						attackTransferMoves.add(new AttackTransferMove(myName, fromRegion, toRegion, armies));
						break;
					}
					else if(toRegion.getPlayerName().equals(myName)) //do a transfer
					{
						attackTransferMoves.add(new AttackTransferMove(myName, fromRegion, toRegion, armies));
						break;
					}
